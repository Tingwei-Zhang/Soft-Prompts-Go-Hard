...
class Attacker:
    ...
    def __init__(self, args, model, targets, instructions, device='cuda:0', is_rtp=False):
    ...
    def attack_partial_constrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255, epsilon = 128/255, rows_to_change=50):
        print('>>> batch_size:', batch_size)
        ...
        # Create a mask for the region to perturb: Only first rows_to_change rows
        mask = torch.zeros_like(img)
        mask[:, :, 0:rows_to_change, :] = 1  # Assuming img is in BCHW format and we alter the first 10 rows
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            with torch.no_grad():
                adv_noise.data = (adv_noise.data - alpha * adv_noise.grad.detach().sign()).clamp(-epsilon, epsilon) * mask
                adv_noise.data = (adv_noise.data + x.data).clamp(0, 1) - x.data
                adv_noise.grad.zero_()
    ...
    def attack_l2(self, text_prompt, img, batch_size=8, num_iter=2000, alpha=1/255, epsilon=30):
        print('>>> batch_size:', batch_size)
        ...
        # Initialize noise within the L2 ball of radius epsilon
        delta = torch.empty_like(img).normal_()
        d_flat = delta.view(delta.size(0), -1)
        n = d_flat.norm(p=2, dim=1).view(delta.size(0), 1, 1, 1)
        r = torch.zeros_like(n).uniform_(0, 1)
        delta *= r / n * epsilon
        adv_noise = delta.to(self.device)
        adv_noise.requires_grad_(True)
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            # Sample a batch of target instructions
            batch_targets = random.sample(self.targets, batch_size)
            batch_indices = [self.targets.index(target) for target in batch_targets]
            selected_instructions = [self.instructions[i] for i in batch_indices]
            modified_prompts = [text_prompt % instruction for instruction in selected_instructions]

            x_adv = x + adv_noise
            x_adv = normalize(x_adv)
            
            # Create prompt with adversarial image
            prompt = prompt_wrapper.Prompt(model=self.model, text_prompts=modified_prompts, img_prompts=[[x_adv]])
            prompt.img_embs = prompt.img_embs * batch_size
            prompt.update_context_embs()

            # Calculate the target loss to be minimized
            target_loss = self.attack_loss(prompt, batch_targets)
            target_loss.backward()

            with torch.no_grad():
                perturbation = adv_noise.grad.data

                # Normalize gradient to ensure L2 norm constraint
                grad_norm = torch.norm(perturbation.view(-1), p=2) + 1e-10
                perturbation = perturbation / grad_norm
                adv_noise.data -= alpha * perturbation  # Gradient descent step

                # Project back to the L2 ball if necessary
                delta = adv_noise.data
                delta_norm = torch.norm(delta.view(-1), p=2)
                factor = epsilon / delta_norm
                factor = min(factor, 1.0)
                adv_noise.data = delta * factor

                adv_noise.data = torch.clamp(x + adv_noise.data, 0, 1) - x
                adv_noise.grad.zero_()
